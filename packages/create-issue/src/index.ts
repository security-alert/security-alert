...
export async function createFromURL(url: string, options: CreatedOptions) {
    ...
    const pattern =
        /^https:\/\/github.com\/(?<owner>[0-9a-zA-Z-_.]+)\/(?<repo>[0-9a-zA-Z-_.]+)\/network\/alert\/(?<filepath>.+)\/(?<pacakgeName>[0-9a-zA-Z-_.]+)\/(open|closed)/;
...
- Package name:  [${targetAlert.securityVulnerability.package.name}](https://www.npmjs.com/package/${
        targetAlert.securityVulnerability.package.name
    })
...
- Patched version: ${
        targetAlert.securityVulnerability.firstPatchedVersion
            ? targetAlert.securityVulnerability.firstPatchedVersion.identifier
            : "none"
    }
- GitHub Alert: <${url}>
${
...
${targetAlert.securityAdvisory.references
    .map((reference) => {
